(function(ns){
	...
	var $regexs = {
		...
		phone : /^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$/,//固定电话号码格式，区号可选   
		mobile : /^((\(\d{2,3}\))|(\d{3}\-))?((13\d{9})|(14[7]\d{8})|(18[012356789]\d{8})|(15[012356789]\d{8}))$/,//手机号码   
		url : /^http:\/\/[A-Za-z0-9]+\.[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"])*$/,//url地址   
		ip : /^(0|[1-9]\d?|[0-1]\d{2}|2[0-4]\d|25[0-5]).(0|[1-9]\d?|[0-1]\d{2}|2[0-4]\d|25[0-5]).(0|[1-9]\d?|[0-1]\d{2}|2[0-4]\d|25[0-5]).(0|[1-9]\d?|[0-1]\d{2}|2[0-4]\d|25[0-5])$/,//ip地址格式   
		currency : /^\d+(\.\d+)?$/,//货币格式   
		number : /^\d+$/,//数字格式   
		zip : /^[1-9]\d{5}$/,//邮政编码格式   
		qq : /^[1-9]\d{4,13}$/,//腾讯QQ号码，5~9位   
		english : /^[A-Za-z]+$/,//英文字母   
		chinese :  /^[\u0391-\uFFE5]+$/,//中文   
		username : /^[a-z]\w{3,19}$/i,//用户名，字母开始，后跟3~19位字符   
		integer : /^[-\+]?\d+$/,//整数格式   
		'double' : /^[-\+]?\d+(\.\d+)?$/,//浮点格式，可正可负，可带小数
		'integerOrFloat' : /^[0-9]+.?[0-9]*$/,//判断字符串是否为数字(整数或者浮点数)
		iszero:/^(?!0$)/,
	...
	ns.FollowMessenger = new Class({// 后跟随式信息提示方式 
	...
	ns.ColorMessenger = new Class({// 文字颜色信息提示方式 
		Extends : AbstractMessenger,
		options : {
			warn : 'red',
			pass : 'green'
	...
	ns.ClassMessenger = new Class({// CSS样式信息提示方式   
		Extends : AbstractMessenger,
		options : {
			warn : 'style-warn',
			pass : 'style-pass'
	...
	ns.TipsMessenger = new Class({// 浮动层信息提示方式
		Extends : AbstractMessenger,
		options : {
			offset : {x : 0, y : 0},
			'class' : 'tooltips'
	...
	ns.AlertMessenger = new Class({// Alert警告信息提示方式 
		Extends : AbstractMessenger,
	...
	ns.SummaryMessenger = new Class({// 汇总信息提示方式
		Extends : AbstractMessenger,
		options : {id : ''},
		showPass : function(o){
	...
	ns.AttributeReader = new Class({// 直接表单项自定义属性配置方式
	...
	ns.TagReader = new Class({// 自定义标签配置方式
		Extends : AbstractReader,
		options : {
			ns : 'ui',
			name : 'v'
	...
	ns.JsonReader = new Class({// JSON数据配置方式  
		Extends : AbstractReader,
		options : {
			wait : false,
	...
	ns.XmlReader = new Class({// XML文件配置方式
	...
	var AbstractElement = new Class({
		...
		getMessage : function(){
			return (this.$json ? this.$json.message : this.options[this.isValid ? 'pass' : (this.value === '' ? 'empty' : 'warn')]) || '\u9A8C\u8BC1\u6B63\u786E';//\28\u672a\u914d\u7f6e\u63d0\u793a\u4fe1\u606f\29  未配置提示信息
		},
		trim : function(text){
	...
	ns.IdCardElement = new Class({
		...
		process : function(){
			...
			if(this.value.length<18) return false;
	...
	ns.CompareElement = new Class({//比较
		Extends : AbstractElement,
		options : {
			to : null,
			toElement : null,
