...
$(function() {
	...
	function addDoc() {
		sy.modalDialog({
			title: '新增发文公文信息'
			, area: ['100%', '100%']
			, content: basePath + 'egovernment/archive/archiveFormIndex?op=add'
			, btn: ['保存', '取消'] //可以无限个按钮
			, btn1: function (index, layero) {
	...
	function addSWDoc() {
		sy.modalDialog({
			title: '新增收文公文信息'
			, area: ['100%', '100%']
			, content: basePath + 'egovernment/archive/archiveswFormIndex'
			, btn: ['保存', '取消'] //可以无限个按钮
			, btn1: function (index, layero) {
				window[layero.find('iframe')[0]['name']].saveFun();
			}
	...
	function editDoc(data) {
	    ...
	    var url;
	    if(type=="2"){
	        url=basePath + 'egovernment/archive/archiveswEditFormIndex';
		}else {
            url=basePath + 'egovernment/archive/archiveFormIndex';
		}
		sy.modalDialog({
			title: '编辑公文信息'
			, area: ['100%', '100%']
	...
	function showDoc(data) {
        ...
        var url;
        if(type=="2"){
            url=basePath + 'egovernment/archive/archiveswEditFormIndex?op=view&archiveid=' + archiveid
        }else {
            url=basePath + 'egovernment/archive/archiveFormIndex?op=view&archiveid=' + archiveid
        }
		layer.open({
