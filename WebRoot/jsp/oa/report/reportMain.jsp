...
$(function() {
	layui.use(['form', 'table', 'layer', 'element', 'laydate'], function () {
		...
		table.render({
			elem: '#docTable'
			, url: basePath + '/oaeamil/queryOareportSend'
			, page: true // 展示分页
			, limit: 10 // 每页展示条数
			...
			, cols: [[
				{field: 'reportcontent', width: 500, title: '工作上报内容', event: 'trclick'}
		...
		var $ = layui.$, active = {
			delPcom: function () {
				if (!table.singleData) {
					layer.alert('请先选择要删除的工作上报！！');
				} else {
					delDoc(table.singleData.oareportid);
				}
			},
			addPcom: function () {
					addDoc();
	...
	function delDoc(oareportid) {
		layer.open({
			...
			, btn1: function (index, layero) {
				$.post(basePath + 'oaeamil/delOareport', {
                         oareportid: oareportid
						},
						function (result) {
							if (result.code == '0') {
								//保证不会重复删除
	...
	function addDoc() {
		sy.modalDialog({
			title: '新增工作上报'
			, area: ['100%', '100%']
			, content: basePath + 'oaeamil/reportFormIndex?op=add'
			, btn: ['保存', '取消'] //可以无限个按钮
			, btn1: function (index, layero) {
    ...
    function hf(data) {
        ...
        url=basePath + 'work/task/gzrwhftaMainIndex?oataskid=' + oareportid
        sy.modalDialog({
            title: '回复记录'
            , area: ['100%', '100%']
            , content: url
	...
	function editDoc(data) {
        var oareportid=data.oareportid;
        url=basePath + 'oaeamil/reportFormIndex?oareportid=' + oareportid
		sy.modalDialog({
			title: '编辑工作上报'
			, area: ['100%', '100%']
			, content: url
	...
	function showDoc(data) {
        var oareportid=data.oareportid;
        url=basePath + 'oaeamil/reportFormIndex?op=view&oareportid=' + oareportid
