...
$(function() {
	layui.use(['form', 'table', 'layer', 'element', 'laydate'], function () {
		...
		table.render({
			elem: '#docTable'
			, url: basePath + '/oaeamil/queryOaemailSend'
			, page: true // 展示分页
			, limit: 10 // 每页展示条数
			...
			, cols: [[
				{field: 'emailcontent', width: 500, title: '站内信内容', event: 'trclick'}
		...
		table.on('tool(tableFilter)', function (obj) {
			...
			if (obj.event === 'trclick') { // 选中行
				if (selectTableDataId != data.oaemailid) {
					// 清除所有行的样式
					...
					selectTableDataId = data.oaemailid;
				} else { // 再次选中清除样式
		...
		var $ = layui.$, active = {
			delPcom: function () {
				if (!table.singleData) {
					...
					delDoc(table.singleData.oaemailid);
				}
			},
			addPcom: function () {
					addDoc();
	...
	function delDoc(oaemailid) {
		layer.open({
			...
			, btn1: function (index, layero) {
				$.post(basePath + 'oaeamil/delOaemail', {
                         oaemailid: oaemailid
						},
						function (result) {
							if (result.code == '0') {
	...
	function addDoc() {
		sy.modalDialog({
			title: '新增站内信'
			, area: ['100%', '100%']
			, content: basePath + 'oaeamil/emailFormIndex?op=add'
			, btn: ['保存', '取消'] //可以无限个按钮
			, btn1: function (index, layero) {
	...
	function editDoc(data) {
        ...
        url=basePath + 'oaeamil/emailFormIndex?oaemailid=' + oaemailid
		sy.modalDialog({
			title: '编辑站内信'
			, area: ['100%', '100%']
			, content: url
	...
	function showDoc(data) {
        ...
        url=basePath + 'oaeamil/emailFormIndex?op=view&oaemailid=' + oaemailid
						...
						<div class="layui-col-md4 layui-col-xs12 layui-col-sm6">
							<div class="layui-form-item">
								...
								<div class="layui-input-inline">
									<input type="text" name="emailcontent" id="emailcontent" class="layui-input">
								</div>
							</div>
						</div>
						<div class="layui-col-md4 layui-col-xs12 layui-col-sm6">
