    ...
    <script type="text/javascript">
        ...
        $(function () {
            layui.use(['form', 'table', 'layer', 'element'], function () {
                ...
                var $ = layui.$, active = {
                    addPcomry: function () { // 添加
                        addPcomry();
                    }
                    , updatePcomry: function () { // 修改
                        if (!table.singleData) {
                            layer.alert('请选择要修改的企业！');
                        } else {
                            updatePcomry(table.singleData.ryid);
                        }
                    }
                    , delPcomry: function () { // 删除
                        if (!table.singleData) {
                            layer.alert('请选择要删除的企业！');
                        } else {
                            delPcomry(table.singleData.ryid);
                        }
                    }
                    , showPcomry: function () { // 查看
                        if (!table.singleData) {
                            layer.alert('请选择要查看的企业！');
                        } else {
                            showPcomry(table.singleData.ryid);
                        }
                    }
                };
                            ...
                            function (result) {
                                if (result.code == '0') {
                                    ...
                                    layer.msg('删除成功', {time: 1000}, function () {
                                        //如果是本页最后一条数据 则返回上一页
                                        if (table.cache.pcomryTable.length == 1) {
                                            curent = curent - 1;
                                        }
                                        refresh('', curent);
                                    });
...
<div class="layui-fluid">
    ...
    <div class="layui-margin-top-15">
        <div class="layui-btn-group demoTable">
            <ck:permission biz="addPcomry">
                <button class="layui-btn" data-type="addPcomry" data="btn_addPcomry">增加</button>
            </ck:permission>
            <ck:permission biz="updatePcomry">
                <button class="layui-btn" data-type="updatePcomry" data="btn_updatePcomry">编辑</button>
            </ck:permission>
            <ck:permission biz="delPcomry">
                <button class="layui-btn layui-btn-danger" data-type="delPcomry" data="btn_delPcomry">删除</button>
            </ck:permission>
            <ck:permission biz="showPcomry">
                <button class="layui-btn " data-type="showPcomry" data="btn_showPcomry">查看</button>
            </ck:permission>
        </div>
        <table class="layui-hide" id="pcomryTable" lay-filter="tableFilter"></table>
    </div>
</div>
</body>
</html>