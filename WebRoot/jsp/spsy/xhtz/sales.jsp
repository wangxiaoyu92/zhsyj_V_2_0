 	...
 	if ($('#qledgerstockid').val().length > 0) {
		...
		function(result) {
			if (result.code=='0') {
				...
				$('#lgprojyrq').val("");	
				$('#lgprojysl').val("");	
				$('#lgprojydwmc2').val(mydata.lgprojydwmc);
				$('#lgprojydwmc').val(mydata.lgprojydwmc);
			} else {
	...
	if (v_retStr != null && v_retStr.length > 0) {
	    for (var k = 0;k <= v_retStr.length-1; k++){	    	
			var myrow = v_retStr[k];
	      	...
	      	$("#lgcomfwnfww").val(myrow.comfwnfww); // 电话号码
			if (myrow.comfwnfww == '1') { // 如果是范围外企业，属于范围外交易
				$("#lgsjylx").val("2");
				$("#lgsjylxms").val("范围外交易");
			} else {
				$("#lgsjylx").val("1");
       		...
       		<table class="table" style="width: 600px;"> 
				...
				<tr>
					...
					<td width="50%"><input id="lgtocommc" name="lgtocommc" style="width: 250px;" 
						...
						<input id="lgcomfwnfww" name="lgcomfwnfww" style="width: 250px;" type="hidden"/>
				...
				<tr>
					<td style="text-align:right;"><nobr>交易类型:</nobr></td>
					<td><input id="lgsjylxms" name="lgsjylxms" style="width: 250px"
							   class="input_readonly" readonly="readonly"/>
						<input id="lgsjylx" name="lgsjylx" style="width: 250px" type="hidden"
							   class="input_readonly" readonly="readonly"/>
					</td>
					<td></td>
				</tr>
