...
$(function() {
		mygrid = $('#mygrid').datagrid({
			toolbar: '#toolbar',
			url: basePath + '/spsy/pesticide/queryPestStock',
	...
	// 新增台账录入
	function addPestStock() {
		var url = basePath + 'spsy/pesticide/pestStockFormIndex';
	...
	// 删除台账信息
	function delPestStock() {
		var row = $('#mygrid').datagrid('getSelected');
		if (row) {
			$.messager.confirm('警告', '您确定要删除该台账信息吗?',function(r) {
				if (r) {
					$.post(basePath + 'spsy/pesticide/delPestStock', {
						qledgerstockid: row.qledgerstockid
					},
					function(result) {
						if (result.code == '0') {
...
function uploadFjView(){
	var row = $('#mygrid').datagrid('getSelected');
	if (row) {
		var url = basePath + "/pub/pub/uploadFjViewIndex?folderName=jinhuopiao&fjwid="+row.qledgerstockid+"&fjtype=12";//12进货索证索票图片
		//创建模态窗口
		parent.sy.modalDialog({
			title : '上传附件',
			width : 900,
        ...
        <div region="center" style="overflow: auto;" border="false">
        	...
        	<sicp3:groupbox title="进货台账列表">
	        	<div id="toolbar">
	        		<table>
						<tr>
							<td><a href="javascript:void(0)" class="easyui-linkbutton" data="btn_addPestStock"
								iconCls="icon-add" plain="true" onclick="addPestStock()">增加</a>
							</td>
							<td>  
								<div class="datagrid-btn-separator"></div>
							</td> 
							<td><a href="javascript:void(0)" class="easyui-linkbutton" data="btn_showPestStock"
								iconCls="ext-icon-application_form_magnify" plain="true" onclick="showPestStock()">查看</a>
							</td>
							<td>  
								<div class="datagrid-btn-separator"></div>
							</td> 
							<td><a href="javascript:void(0)" class="easyui-linkbutton" data="btn_delPestStock"
								iconCls="icon-remove" plain="true" onclick="delPestStock()">删除</a>
							</td>
							<td>
								<div class="datagrid-btn-separator"></div>
							</td>
							<td><a href="javascript:void(0)" class="easyui-linkbutton" data="btn_uploadFjView"
								   iconCls="ext-icon-application_form_magnify" plain="true" onclick="uploadFjView()">交易附件管理</a>
							</td>
							<td>  
								<div class="datagrid-btn-separator"></div>
							</td> 																																													
