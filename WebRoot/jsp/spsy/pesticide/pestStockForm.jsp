...
$(function() {
	...
	if ($('#qledgerstockid').val().length > 0) {
		...
		$.post(basePath + 'spsy/pesticide/queryPestStock', {
	...
	var submitForm = function() {
		...
		var url = basePath + 'spsy/pesticide/savePestStock';
...
function myselectcom(){
    var url = basePath + 'jsp/spsy/pesticide/selectGhsCom.jsp'; // 查询供货商企业
...
// 选择商品
function selectPesticide(){
	var comid = $("#lgfromcomid").val();
	if (comid == "" || comid == null || comid.length == 0) {
		alert("请先选择进货商！！！");
		return;
	}
    var url = basePath + 'jsp/spsy/pesticide/selectPesticide.jsp';
	var dialog = parent.sy.modalDialog({
			title : '选择药品或化肥',
			param : {
				comid : comid,
	...
	<form id="stockAddDlgfm" name="stockAddDlgfm" method="post">
		<input id="qledgerstockid" name="qledgerstockid" type="hidden" value="<%=v_qledgerstockid%>"/>
       	<table class="table" style="width: 600px;">
			...
			<tr>
				...
				<td><% if(!"view".equalsIgnoreCase(op)){ %>
					<a id="btnselectcom" href="javascript:void(0)" class="easyui-linkbutton"
						iconCls="icon-search" onclick="myselectcom()">选择供货商 </a>
					<%} %>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><% if(!"view".equalsIgnoreCase(op)){ %>
						<a id="btnselectpro" href="javascript:void(0)" class="easyui-linkbutton"
							iconCls="icon-search" onclick="selectPesticide()">选择农药或化肥 </a>
							<%} %>						
				</td>
			</tr>
			<tr>
				<td style="text-align:right;" ><nobr>农药或化肥条码:</nobr></td>
			...
			<tr>
				<td style="text-align:right;" ><nobr>批号批次:</nobr></td>
			...
			<tr>
				<td style="text-align:right;">溯源码:</td>
				<td>
