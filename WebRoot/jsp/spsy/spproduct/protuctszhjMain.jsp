...
var v_scdwdm = <%=SysmanageUtil.getAa10toJsonArray("CPPCSCDWDM")%>;
		...
		mygrid = $('#mygrid').datagrid({
			...
			columns : [ [
	        {
	            ...
	            formatter:function(value,rec){ 
					  var v_ret = '<a href="javascript:addSzhjxx(\''+rec.cppcpch+'\')" mce_href="#">'
					  	+'<img src="<%=basePath%>images/pub/edit_add.png" align="absmiddle">添加生长环境信息</a>&nbsp;'
					  	+'<a href="javascript:showSzhjxx(\''+rec.cppcpch+'\')" mce_href="#">'
					  	+'<img src="<%=basePath%>images/pub/doc2.png" align="absmiddle">查看生长环境信息</a>'	;
	                  return  v_ret; 
	             }   
	        }
			] ]
	...
	// 添加生长环境信息
	function addSzhjxx(cppcpch) {
		var v_productid = $("#productid").val();
		var url = basePath + 'spsy/spproduct/szhjxxFromIndex';
	...
	// 查看生长环境信息
	function showSzhjxx(cppcpch) {
		var v_productid = $("#productid").val();
		var url = basePath + 'spsy/spproduct/showSzhjxxFromIndex';
		parent.sy.modalDialog({
				title : '查看生长环境信息',
				param : {
					proid : v_productid,
