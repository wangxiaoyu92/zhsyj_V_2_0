...
<script type="text/javascript">
	...
	function uploadJinhuopiao() {
		...
		if (row) {	
		    var dialog = parent.sy.modalDialog({
				...
				width : 900,
				height : 700,
				url : basePath + "/pub/pub/uploadFjViewIndexEasyui?folderName=jinhuopiao&fjtype=12&fjwid="+row.hjhbid
			},function(dialogID){
			...
			$.messager.alert('提示', '请先选择要索证索票的记录！', 'info');
		}    
	};	
	...
	function uploadJiancepic(prm_jgztlx) {
		...
		if (row) {
		    var dialog = parent.sy.modalDialog({
				...
				width : 900,
				height : 700,
				url : basePath + "pub/pub/uploadFjViewIndexEasyui?folderName=jchhgzm&fjtype=9&fjwid="+row.hjhbid
			},function(dialogID){
				var obj = sy.getWinRet(dialogID);
				if(obj.type == 'ok'){
					$('#mygrid').datagrid('reload'); 
	...
	function addjldw(){
		...
		var url = "<%=basePath%>jsp1.0/sysmanager/aa10.jsp";
	    var dialog = parent.sy.modalDialog({ 
				title : ' ',
				width : 860,
				height : 460,
