...
/*		getbqsx();
	...
	//许可证到期预警
	function comXKZYuJing(){
		var obj = new Object();
		var url = basePath+'common/sjb/comXKZYuJingIndex?time='+new Date().getMilliseconds();
		var dialog = parent.sy.modalDialog({
			title : '许可证到期预警',
			width : 1000,
			height : 560,
	...
	function XKZYJ(){
		$.post(basePath + 'common/sjb/daoQiYuJing', {
				},
				function(result) {
					if (result.code=='0') {
						$('#xkzdqyj').html(result.total);
					} else {
						parent.$.messager.alert('提示','查询失败：'+result.msg,'error');
					}
					parent.$.messager.progress('close');
	...
	function comRYYuJing(){
		var obj = new Object();
		var url = basePath+'pcomry/pcomRYYuJingIndex?time='+new Date().getMilliseconds();
		var dialog = parent.sy.modalDialog({
			title : '许可证到期预警',
			width : 1000,
			height : 560,
	...
	function RYYJ(){
		$.post(basePath + 'pcomry/pcomryYuJing', {
				},
				function(result) {
					if (result.code=='0') {
						$('#comryyj').html(result.total);
					} else {
						parent.$.messager.alert('提示','查询失败：'+result.msg,'error');
					}
					parent.$.messager.progress('close');
			...
			<tr align="center" >				
				<td width="50%" valign="top" height="100%">
					<div id="p1" class="easyui-panel" title="平台介绍"     
				        style="height: 440px;padding:0px;background:#fafafa;"
