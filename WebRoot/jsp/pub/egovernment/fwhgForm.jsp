    ...
    $.ajax({
        ...
        success:function(result){
            ...
            if(swzbh=='2') {
                v_transname="合法性审查";
                v_transnamea="合法性审查";
                var v_url2 = encodeURI(encodeURI("<%=basePath%>workflow/doWffwprocess?ywlcid=<%=v_ywlcid%>&transval=" + v_transval + "&shifouTongguo=" + v_shifouTongguo+ "&ywlcuserid=" + cyryid + "&transnamea=" + v_transnamea+ "&transname=" + v_transname + "&transyy=" + v_transyy + "&time=" + new Date().getMilliseconds()));
        ...
        <td width="88%" align="left">
	     <div id="xuanzeliuxiang">
		     <%-- <%for (Wf_node_trans v_Wf_node_trans:v_listWf_node_trans){%>
		      ...
		      <%} %>   --%>
			 <div id="a">
				 <input type="radio" name="transname" <%=v_nextchecked%> id="transname"  value="签发" style="cursor: pointer;"/>签发
			 </div>
				 <div id="b">
				 <input type="radio" name="transname1" <%=v_nextchecked%> id="transname1"  value="合法性审查" style="cursor: pointer;"/>合法性审查
				 </div>
	     </div>	         
        </td>
      </tr>
