...
<%
    ...
    String fjtype = StringHelper.showNull2Empty(request.getParameter("fjtype"));
	//上传文件存放的目标文件夹名称
                ...
                if (uploader.files.length > 0) {// 判断队列中是否有文件需要上传
                    uploader.bind('StateChanged', function() {// 在所有的文件上传完毕时，提交表单
                        if (uploader.files.length === (uploader.total.uploaded + uploader.total.failed)) {
                            ...
                            $.ajax({
                                url: basePath + '/pub/pub/saveFj?fjwid='+fjwids+'&fjtype='+fjtype,
                                type: 'post',
                                async: true,
                                cache: false,
                                timeout: 100000,
        ...
        function showUploadFj(){
            if(fjpath.length>0){
                for(var i=0;i<fjpath.length;i++){
                    ...
                    $('#picbox').append("<div style=\"float:left;text-align:center;margin:0 20px 20px 0;\"><a onclick=showPic(" + sy.contextPath + fjpath[i] + "); ><img width=\"200px\" height=\"150px\"style=\"padding:2px;border:1px solid #ccc;\" src=\"" + sy.contextPath +  fjpath[i] + "\"/></a></div>");
                }
            }
        }
