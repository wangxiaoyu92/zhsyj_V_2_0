...
<% 
    ...
    if (request.getAttribute("mybean") != null) {
    	localZfwsjygzs14DTO = (Zfwsjyjcjyjdgzs14DTO) request.getAttribute("mybean");
    }	
%>
<html>
<head>
...
function myprint(){
	...
	var url="<%=basePath%>pub/wsgldy/zfwsjygzsPrintIndex?ajdjid="
...
<body class="b1 b2 zfwsbackgroundcolor">
	<form id="myform" method="post">
		...
		<div align="right">
			...
			<input type="text" id="jygzwsbh" name="jygzwsbh" class="easyui-validatebox" 
			data-options="required:true" style="width: 260px;text-align: right;"
			<%if(localZfwsjygzs14DTO.getJygzwsbh()!=null && 
				!"".equals(localZfwsjygzs14DTO.getJygzwsbh())){%> 
			value="${mybean.jygzwsbh}"<%}else{%>value="（××）食药监×检告〔年份〕×号"<%}%>/>
			</span>
			</p>
		</div>
		...
		<p class="p4">
			...
			class="zfwsInputtextReadonly" readonly="readonly" value="${mybean.commc}"/>：</span>
		</p>
		<p class="p5">
			<span class="s2">我局决定对<input type="text" id="jygzjzwsbh" name="jygzjzwsbh" 
			placeholder="记载文书编号" style="width: 200px;text-align: left;" class="easyui-validatebox" 
			data-options="required:true" value="${mybean.jygzjzwsbh}"/>
			<input type="text" id="jygzjzwsmc" name="jygzjzwsmc" style="width: 200px;text-align: left;" 
			placeholder="记载文书名称" class="easyui-validatebox" data-options="required:true" 
			value="${mybean.jygzjzwsmc}"/>
			所记载的物品进行检验（检测、检疫、鉴定），检验（检测、检疫、鉴定）期限自
			<input name="jygzksrq" id="jygzksrq" placeholder="开始日期" class="Wdate" 
			onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd'})" class="easyui-validatebox" 
			data-options="required:true" style="width: 175px;" value="${mybean.jygzksrq}" >
			&times;年&times;月&times;日至
			<input name="jygzjsrq" id="jygzjsrq" placeholder="结束日期" class="Wdate" 
			onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd'})" class="easyui-validatebox" 
			data-options="required:true" style="width: 175px;" value="${mybean.jygzjsrq}" >
		...
		<div align="right">
			...
			<p class="p3">
				<span class="s2">&times;年&times;月&times;日
					...
					class="easyui-validatebox" data-options="required:true" style="width: 175px;" 
					value="${mybean.jygzgzrq}" >
				</span>
			</p>
		</div>
		<p class="p11"></p>
