...
var v_wsPrintType = <%=SysmanageUtil.g_WSPRINTTYPE%>;//文书打印类型0正常打印1套打
var s = new Object();
s.type = "";   //设为空不刷新父页面
sy.setWinRet(s);
var mygrid;
...
function myprint(){
    ...
    if(sjws=='2'){
        if (v_wsPrintType=="1"){//套打
/*            var url2="<%=basePath%>/common/sjb/getWsPrintPath?time="+new Date().getMilliseconds();
            $.post(url2, {
                    ajdjid:v_ajdjid,
			        fjwid:xcjcid
                },
                function(result) {
                    if (result.code=='0') {
                       // var v_fjpath = result.fjpath;
                        self.location='http://www.baidu.com';
                    } else {
                        parent.$.messager.alert('提示','查询失败：'+result.msg,'error');
                    }
                }, 'json');*/
            //var url2="<%=basePath%>/common/sjb/getWsPrintPath?ajdjid="+v_ajdjid+"&fjwid="+xcjcid+"&time="+new Date().getMilliseconds();
            var url2="printpdf:ajdjid="+v_ajdjid+"&fjwid="+xcjcid+"&";
            //var url2={"printpdf":'1',"ajdjid":v_ajdjid,"fjwid":xcjcid};
            self.location=url2;
		}else {
		...
		parent.sy.modalDialog({
            title: '打印',
            area: ['100%', '100%']
            , content: basePath + 'pub/wsgldy/zfwsxcjcblPrintIndex'
            , param: {
                ajdjid : v_ajdjid,
                zfwsqtbid : xcjcid,
...
function saveAsMoban(){
	...
	parent.sy.modalDialog({
        title: '另存模板',
        area: ['100%', '100%']
        , content: basePath + 'pub/wsgldy/zfwsmobanIndex'
        , param: {
            ajdjid : v_ajdjid,
            zfwsdmz : v_zfwsdmz,
            time : new Date().getMilliseconds()
...
<div style="width: 210mm; margin: 0 auto">
    ...
    <form id="myform" method="post">
		...
		<p class="p5">
			...
			<input type="radio" name="xcjcsfsqhb" value="1"
			<% if (dto.getXcjcsfsqhb() != null &&
				"1".equals(dto.getXcjcsfsqhb())) {%>
			checked="checked"<%}%>>，
			否<input type="radio" name="xcjcsfsqhb" value="0"
