...
<%@ page import="com.zzhdsoft.mvc.GlobalConfig,com.zzhdsoft.utils.SysmanageUtil,com.askj.zfba.dto.Zfwsxwdcbl7DTO"%>
...
<%
	...
	String v_ajzfwsid = URLDecoder.decode(StringHelper.showNull2Empty(request.getParameter("ajzfwsid")),"UTF-8");
	//是否可以保存0否1是
	...
	if (request.getAttribute("mybean") != null) {
		dto = (Zfwsxwdcbl7DTO) request.getAttribute("mybean");
	}
	String sjws=null;
...
function mysave() {
		...
		$('#myform').form(
			'submit',
			{
				url :  basePath + '/pub/wsgldy/savezfwsxwdcbl?sjordn=<%=sjws%>',
				onSubmit : function() {
...
function myprint(){
	...
	if('<%=sjws%>'=='2'){
    	var url="<%=basePath%>/common/sjb/getajdjDocumentsHtml?ajdjid="
		 	+v_ajdjid+"&zfwsqtbid="+$('#xwdcblid').val()+"&type="+'3'+"&time="+new Date().getMilliseconds();
   		self.location=url; 
    }else{   
		var url = basePath + "pub/wsgldy/zfwsxwdcblPrintIndex";
		//创建模态窗口
...
function bdcrqzrqOnBlur(){//被调查人签字 日期 
	var v_dcblbdcrqzrq=$("#dcblbdcrqzrq").val();
	if (v_dcblbdcrqzrq!=null && v_dcblbdcrqzrq!=""){
		$("#dcbldcrqzrq").val(v_dcblbdcrqzrq);
		$("#dcbljlrqzrq").val(v_dcblbdcrqzrq);
	}
}
	...
	<body class="b1 b2 zfwsbackgroundcolor">
		<form id="myform" method="post">
			...
			<p class="p5">
				...
				<span class="s4"> <input id="dcblbxwrxb"
					name="dcblbxwrxb" style="width: 60px;" value="${mybean.dcblbxwrxb}" /> </span>
				<span class="s2">民族：</span> <span class="s4"> <input id="dcblmz"
					name="dcblmz" value="${mybean.dcblmz}" style="width: 100px;" /> </span> 
			...
			<p class="p5">
				...
				<span class="s4"> <input id="dcblbdcryb" name="dcblbdcryb" style="width: 150px;"
					value="${mybean.dcblbdcryb}" /> </span> 
			</p>
			<p class="p5">
			...
			<p class="p7">
				<span class="s2">答： <input name="dcblsfkqc" id="dcblsfkqc"
					...
					<% if(dto.getDcblsfkqc() == null || "".equals(dto.getDcblsfkqc())){ %>
					value="我已看清楚。"
			...
			<p class="p7">
				<span class="s2">答： <input name="dcblsfsqdcryhb"
					...
					<% if(dto.getDcblsfsqdcryhb() == null || "".equals(dto.getDcblsfsqdcryhb())){ %>
					value="不申请回避。"
			...
			<p class="p8">
				<span class="s2">答： <input name="dcblsfmbbnzwz"
					...
					<% if(dto.getDcblsfmbbnzwz() == null || "".equals(dto.getDcblsfmbbnzwz())){ %>
					value="明白。"
				...
				<span class="s2">调查记录：
					<p>
						<textarea id="dcbldcjl" name="dcbldcjl" style="width: 800px;height: 200px;"
							data-options="required:false ">${mybean.dcbldcjl}</textarea>
					</p> </span>
			</p>
			<p class="p9"></p>
			<p class="p9"></p>
			...
			<p class="p4">
				...
				<span class="s2"> 年 月 日 <input name="dcblbdcrqzrq"
					...
					onblur="bdcrqzrqOnBlur();"
					readonly="readonly" style="width: 175px;"
					<% if(dto.getDcblbdcrqzrq() != null){ %>
			...
			<p class="p4">
				<span class="s2"> 调查人签字：</span>  <!-- 调查人员签字 既是 执法人员 -->
				...
				<span class="s2">年 月 日 <input name="dcbldcrqzrq" id="dcbldcrqzrq" class="Wdate"
					onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd'})"
					readonly="readonly" style="width: 175px;"
					<% if(dto.getDcbldcrqzrq() != null){ %>
					value="${mybean.dcbldcrqzrq}"
					<%} else { %> value="<%=SysmanageUtil.getDbtimeYmd()%>" <%} %>> 
				</span>
			</p>
			<p class="p4">
				<span class="s2"> 记录人签字：</span>  <!-- 调查人员签字 既是 执法人员 -->
				<span class="s4"> <input id="dcbljlrqz" name="dcbljlrqz"
					style="width: 90px;" value="${mybean.dcbljlrqz}" />
				</span>
				<span class="s2">年 月 日 <input name="dcbljlrqzrq" id="dcbljlrqzrq" class="Wdate"
					onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd'})"
					readonly="readonly" style="width: 175px;"
					<% if(dto.getDcbljlrqzrq() != null){ %>
					value="${mybean.dcbljlrqzrq}"
					<%} else { %> value="<%=SysmanageUtil.getDbtimeYmd()%>" <%} %>> 
				</span>
			</p>
			<p class="p10"></p>
