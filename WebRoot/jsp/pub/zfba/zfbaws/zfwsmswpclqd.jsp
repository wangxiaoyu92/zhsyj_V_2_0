...
<% 
    ...
    if (request.getAttribute("mybean") != null) {
    	localZfwsmswpclqd31DTO = (Zfwsmswpclqd31DTO) request.getAttribute("mybean");
    }	
%>
<html>
<head>
...
function myprint(){ 
    ...
    var v_mswpclqdid=$("#mswpclqdid").val(); 
	var url="<%=basePath%>pub/wsgldy/zfwsmswpclqdPrintIndex?ajdjid="+v_ajdjid+"&zfwsqtbid="+v_mswpclqdid+"&time="+new Date().getMilliseconds(); 
...
<body class="b1 b2 zfwsbackgroundcolor">
	<form id="myform" method="post">
		...
		<input id="mswpclqdid" name="mswpclqdid" value="${mybean.mswpclqdid}" />
		...
		<div align="right">
			<p class="p3"><span class="s2">                               
				<input type="text" id="msclwsbh" name="msclwsbh" class="easyui-validatebox" 
				data-options="required:true" style="width: 260px;text-align: right;"
				<%if(localZfwsmswpclqd31DTO.getMsclwsbh()!=null && !"".equals(localZfwsmswpclqd31DTO.getMsclwsbh())){%> 
				value="${mybean.msclwsbh}"<%}else{%>value="（××）食药监×物处〔年份〕×号"<%}%>/>
				</span>
			</p>
		</div>
		...
		<p class="p7">
			...
			class="Wdate" placeholder="特邀参加人签字日期"
			...
			class="Wdate" placeholder="承办人签字日期"
			onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd'})" 
