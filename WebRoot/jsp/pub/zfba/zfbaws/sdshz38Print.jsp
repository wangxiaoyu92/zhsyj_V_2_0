...
function CreatePrintPage() {
	...
	LODOP.PRINT_INITA(0,0,"210mm","297mm","执法文书——送达回执");
...
<body class="b1 b2">
	...
	<div align="center" id="page1">
		...
		<p class="p3">
			<span class="s3">受送达单位（人）：</span>
			<span style="border-bottom: 0px solid  white-space;display: inline-block;">
			<%if (dto.getSdhzssddw() != null) { %> <%=dto.getSdhzssddw()%><%} %></span>
		</p>
		<p class="p3" style="min-height: 100px; line-height: 24px;">
			<span class="s3">送达文书名称及文书编号：</span><br>
			<%if(dto.getSdhzwsmcbh() != null && !"".equals(dto.getSdhzwsmcbh())){ %>
				<span><%=SysmanageUtil.replaceStrChuLast(dto.getSdhzwsmcbh())%></span><%}%>
		</p>
		<p class="p3">
			<span class="s3">送达方式：</span>
			<span style="border-bottom: 0px solid  white-space;width: 283px;display: inline-block;">
			<%if (dto.getSdhzsdfsstr() != null) { %><%=dto.getSdhzsdfsstr()%><%} %></span>
			<span class="s3">送达地点：</span>
			<span style="border-bottom: 0px solid  white-space;display: inline-block;">
			<%if (dto.getSdhzsddd() != null) { %><%=dto.getSdhzsddd()%><%} %></span>
		</p>
		<p class="p3">
			<span class="s3">送达人：</span>
			<span style="border-bottom: 0px solid  white-space;width: 293px;display: inline-block;">
			<%if (dto.getSdhzsdr() != null) { %> <%=dto.getSdhzsdr()%><%} %></span>
			<span class="s3">送达日期:</span>
			...
			<%if(dto.getSdhzsdqzrq() != null && !"".equals(dto.getSdhzsdqzrq())){ %>
			<span class="s3"><%=dto.getSdhzsdqzrq().substring(11,13)%></span><%} else {%>
			<span style="border-bottom: 0px solid  white-space;width: 30px;display: inline-block;"></span><%} %>
			<span class="s3">时</span>
			<%if(dto.getSdhzsdqzrq() != null && !"".equals(dto.getSdhzsdqzrq())){ %>
			<span class="s3"><%=dto.getSdhzsdqzrq().substring(14,16)%></span><%} else {%>
			<span style="border-bottom: 0px solid  white-space;width: 30px;display: inline-block;"></span><%} %>
			<span class="s3">分</span>
		</p>
		<p class="p3">
			<span class="s3">受送达单位（人）：</span>
			<span style="border-bottom: 0px solid  white-space;width: 225px;display: inline-block;">
				<%if (dto.getSdhzssddwqz() != null) { %> <%=dto.getSdhzssddwqz()%><%} %></span>
			<span class="s3">送达日期:</span>
			...
			<span class="s3">日</span>
			<%if(dto.getSdhzssddwqzrq() != null && !"".equals(dto.getSdhzssddwqzrq())){ %>
			<span class="s3"><%=dto.getSdhzssddwqzrq().substring(11,13)%></span><%} else {%>
			<span style="border-bottom: 0px solid  white-space;width: 30px;display: inline-block;"></span><%} %>
			<span class="s3">时</span>
			<%if(dto.getSdhzssddwqzrq() != null && !"".equals(dto.getSdhzssddwqzrq())){ %>
			<span class="s3"><%=dto.getSdhzssddwqzrq().substring(14,16)%></span><%} else {%>
			<span style="border-bottom: 0px solid  white-space;width: 30px;display: inline-block;"></span><%} %>
			<span class="s3">分</span>
		</p>
		<br><br><br><br><br><br>
		<hr style="height:2px;border:none;border-top:2px solid #555555;clear:both;">
		<p class="p5" style="min-height: 300px; line-height: 24px;">
			<span class="s3">备注：</span><br>
			<%if(dto.getSdhzbz() != null && !"".equals(dto.getSdhzbz())){ %>
				<span><%=SysmanageUtil.replaceStrChuLast(dto.getSdhzbz())%></span><%} %>
		</p>
		<hr style="height:2px;border:none;border-top:2px solid #555555;clear:both;">
	</div>
</body>
