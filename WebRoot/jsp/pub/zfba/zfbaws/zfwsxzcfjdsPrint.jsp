...
function CreatePrintPage() {
	...
	LODOP.ADD_PRINT_HTM("500mm","20mm","85%","100%","注：正文3号仿宋体字，存档（1）。");
	LODOP.NewPage();
	var strBodyStyle="<style>"+document.getElementById("sty1").innerHTML+"</style>";
	LODOP.ADD_PRINT_HTM("20mm","20mm","RightMargin:1.8cm","BottomMargin:2.2cm",strBodyStyle+document.getElementById("page1").innerHTML);
	LODOP.ADD_PRINT_HTM("500mm","20mm","85%","100%","必要时交<%=localZfwsxzcfjds28DTO.getQzzzrmfy() %>人民法院强制执行（2）。");
}
</script>
...
<body class="b1 b2">
	...
	<div align="center" id="page1">
		...
		<p class="p4">
			<span class="s2">当事人：<%=localZfwsxzcfjds28DTO.getCfjddsr()%></span>
		</p>
		<p class="p4">
			<span class="s2">地址（住址）：<%=StringHelper.myMakeStrLen(
				localZfwsxzcfjds28DTO.getCfjddz(),70,0)%>
				邮编：<%=localZfwsxzcfjds28DTO.getCfjdyb() %></span>
		</p>
		<p class="p4">
			<span class="s2">营业执照或其他资质证明：<%=StringHelper.myMakeStrLen(
				localZfwsxzcfjds28DTO.getCfjdyyzz(),58,0)%>
			编号：<%=localZfwsxzcfjds28DTO.getCfjdyyzzbh() %></span>
		</p>
		<p class="p4">
			<span class="s2">组织机构代码（身份证）号：<%=localZfwsxzcfjds28DTO.getCfjdzzjgdm() %></span>
		</p>
		<p class="p4">
			<span class="s2">法定代表人（负责人）：<%=StringHelper.myMakeStrLen(
				localZfwsxzcfjds28DTO.getCfjdfddbr(),30,0)%>
			性别：<%=StringHelper.myMakeStrLen(localZfwsxzcfjds28DTO.getCfjdfddbrxbstr(),18,0)%>
			职务：<%=localZfwsxzcfjds28DTO.getCfjdfddbrzw() %></span>
		</p>
		<p class="p4">
