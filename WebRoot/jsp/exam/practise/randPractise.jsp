...
<script type="text/javascript">
	...
	function queryQuestionInfo() {
		var param = { "page" : currPage, "rows" : pageSize, "continueFlag" : continueFlag, "answerType" : "1" };
		$.ajax({
			type : "POST",
		   	url : basePath + "exam/practise/queryRandPractiseQuestions",
		   	data : param,
		   	traditional : true,
		   	success : function(result) {
				...
				if (result.code == "0"){
					...
					v_type = result.qsnList[currQsnIndex].info.qsnInfoType;
					qsnInfo = result.qsnList[currQsnIndex].info;
					dataList = result.qsnList[currQsnIndex].dataList;
	...
	function nextQusetion () {
		var maxQsnNo = pageSize; // 当前最大试题数
		if (currQsnIndex < maxQsnNo) {
			if (qsnInfoList[currQsnIndex]) {
				v_type = qsnInfoList[currQsnIndex].info.qsnInfoType;
				qsnInfo = qsnInfoList[currQsnIndex].info;
				dataList = qsnInfoList[currQsnIndex].dataList;
			}
			++currQsnIndex;
			showQsnOneByOne();
		} else {
	...
	function flushOrLeave() {
		$.ajax({
			type : "POST",
		   	...
		   	data : { "qsnInfoId" : v_answeredQsn, "answerType" : "1" },
		   	traditional : true,
		   	success : function(result){
