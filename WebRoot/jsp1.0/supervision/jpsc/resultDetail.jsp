...
var result = getTbodyInfo("JPSCQY_checkPaln","1");
if(result){
getresults();
}
			...
			function(result) {
				if (result.code=='0') {
					...
					if(mydata[i].detaildecide=="1"){
						...
						$('#resultstr').before("<tr  id='tr"+i+"' ><td colspan='2'>"+mydata[i].content+"</td><td>("+isFuhe+")</td></tr>");
						}else{//ID没有存在（单元格合并）
						$('#resultstr').before("<tr id='total"+i+"' ><td id="+mydata[i].itemid+" rowspan="+mydata[i].count+">"+mydata[i].itemname+"</td><td colspan='2' >"+mydata[i].content+"</td><td>("+isFuhe+")</td></tr>");
						}
					}
					}
// 					加载ckeditor
...
// 					var endtime = '${obj.map.data.resultdate}';
					...
					.replace("checktime",starttime).replace("checktype","日常检查");
					$('#editor').html(html);
					$('#resultstr').append(mydata.tfootinfo);
					}
			}, 'json');
			return true;
				}
	//获取表尾(没用)
 function getTbodyBottomInfo(str,value){
$.post(basePath + 'supervision/checkresult/getTbodyInfo', {
				tbodytype : str,
				tbodycode:value
			}, 
