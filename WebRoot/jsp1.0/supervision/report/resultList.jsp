...
var grid;
var aaa027name = sy.getUrlParam("aaa027name");
var daleiname = sy.getUrlParam("daleiname");
var year = sy.getUrlParam("year");
var resultstate = sy.getUrlParam("resultstate");
var param = { 
	...
	var url = "<%=basePath%>/supervision/checkresult/resultReportlist" ;
	$(function() {
		grid = $('#grid').datagrid({
			url : url,
			queryParams : param,
		...
		if(row){
			if(row.resultstate == "1"){//未检查完毕
				var dialog = parent.sy.modalDialog({
					...
					width :1040,
				    height :560,
					url : basePath + '/supervision/checkresult/resultdetail?resultid='
						+row.resultid+'&resultstate='+row.resultstate+'&planchecktype='
						+row.planchecktype+'&comdalei='+row.comdalei,
					buttons : [ 
					//没有完成去除保存
		// 			{
		// 				text : '保存',
	        ...
	        <form id="fm" name="fm" method="post">		
				<table class="table" style="width: 1000px;">
					...
					<tr>
						...
						<td><input type="text" id="operatedate" name="operatedate"    onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'operateenddate\')}',readOnly:true})" class="Wdate" readonly="readonly"/> 					
						&nbsp;-&nbsp; <input type="text" id="operateenddate" name="operateenddate"    onFocus="WdatePicker({minDate:'#F{$dp.$D(\'operatedate\')}',readOnly:true})" class="Wdate" readonly="readonly"/> 
