...
<script type="text/javascript">
		...
		grid = $('#grid').datagrid({
			...
			columns : [ [ {
	            ...
	            formatter:function(value,row,index){
	                ...
	                str += '<a href="javascript:editNews(' + row.newsid 
	                	+ ')"><img src="<%=contextPath %>/jslib/jquery-easyui-1.3.4/themes/icons/modify.gif" align="absmiddle">修改</a>&nbsp;&nbsp;'; 
	...
	// 新增
	function addNews() {
		var url = "<%=contextPath %>/news/addnews";
		//创建模态窗口
	...
	function editNews(newsid) {		
		var row = $('#grid').datagrid('getSelected');
		if (row) {
			var url = "<%=contextPath %>/news/editnews?newsid=" + row.newsid;
			//创建模态窗口
			var dialog = parent.sy.modalDialog({
				title : '新增',
				width : 900,
	...
	function delNews(newsid) {
		...
		if (row) {
			var sfyx = row.sfyx;
			if (sfyx == '1') {
				$.messager.alert('警告', '该新闻当前处于有效状态，不可删除！', 'warning');
				return;
			}
			$.messager.confirm('警告', '您确定要删除这条记录吗，这将删除对应的新闻、附件，且不可恢复！',function(r) {
				if (r) {
