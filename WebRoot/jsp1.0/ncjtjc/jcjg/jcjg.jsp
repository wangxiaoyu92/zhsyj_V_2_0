...
<script type="text/javascript">
	...
	$(function() {
		...
		grid = $('#grid').datagrid({
			...
			onClickRow : function(rowIndex, rowData){			
				...
				jcsbjgy_grid.datagrid({
					url : basePath + '/ncjtjc/jcgl/queryJcjgy',			
					queryParams : { 'jcsbid':jcsbid }
				});
			}
		});

		jcsbjgy_grid = $('#jcsbjgy_grid').datagrid({
			//title: '已指派的现场监管员',
			//iconCls: 'icon-tip',
	...
	// 选择监管员
	var selectJcjgy = function() {
		var row = $('#grid').datagrid('getSelected');
		if (row) {
			var v_jcsbid = row.jcsbid;		    
		    var url = basePath + '/jsp/pub/pub/selectJcjgy.jsp'; 
		    /* var obj = new Object();
			var k = popwindow(url,obj,950,500); */
	...
	function uploadFuJian(){
		...
		if (row) { 
		    ...
		    var v_dmlb = "JCXCJCZDFJ";//附件参数小类编号：聚餐现场检查指导附件
	...
	//打印
	function printJcxczdb(){
		var row = $('#grid').datagrid('getSelected');
		if (row) {
			var params = "jcsbid=" + row.jcsbid;
			sy.doPrint('ncjtjc/jcxczdb.cpt',params);
		}else{
			$.messager.alert('提示', '请先选择要操作的记录！', 'info');
		}
	}
        ...
        <div region="center" style="overflow: true;" border="false">
        	...
        	<sicp3:groupbox title="聚餐申报列表">
	        	<div id="toolbar">
	        		<table>
						<tr>	        		
							<td><a href="javascript:void(0)" class="easyui-linkbutton" data="btn_addJcjgy"
								iconCls="ext-icon-group_add" plain="true" onclick="selectJcjgy()">指派现场监管员</a>
							</td>
							<td>  
...
<%--								iconCls="icon-print" plain="true" onclick="printJcxczdb()">打印《农村集体聚餐食品安全现场指导表》</a>--%>
<%--							</td>--%>
<%--							<td>  --%>
<%--								<div class="datagrid-btn-separator"></div>--%>
<%--							</td>   --%>
	        ...
	        <sicp3:groupbox title="已指派的【现场监管员】信息">				
		        <div id="jcsbjgy_grid" ></div>
		    </sicp3:groupbox> 
        </div>        
    </div>    
</body>
