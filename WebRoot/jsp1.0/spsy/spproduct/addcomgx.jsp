...
s.type = "ok";   // 设为刷新父页面
	...
	// 公司供销类型
	//var v_comgxlx = <%=SysmanageUtil.getAa10toJsonArrayXz("COMGXLX")%>;
	...
	$(function() {
		...
		grid1 = $('#grid1').datagrid({
			title: '本企业产品材料表',
			url : basePath + '/spsy/productin/queryProductin?procomid=<%=comid%>',
			iconCls: 'icon-tip',
		...
		grid2 = $('#grid2').datagrid({
			title: '供销商企业信息表',
			iconCls: 'icon-tip',
			url : basePath + '/pcompany/queryCompany?jgztfwnfww=1',
			striped : true,// 奇偶行使用不同背景色
			singleSelect : true,// True只允许选中一行
			pagination : true,// 底部显示分页栏
			pageSize : 10,
	...
	function addComProducts() { 
		...
		if (!row) {
			...
			if (row2) {
				...
				$.messager.alert('操作提醒', '请选择供销商的产品或材料!', 'info',function(){
					return;
				});
			}
		}
						...
						<td>
						   <table>
						     ...
						     <tr>
		                        ...
		                        <input type="radio" value="0" name="comfwnfww" id="comfwnfww" />范围内&nbsp;
		                        <input type="radio" value="1" name="comfwnfww" id="comfwnfww" checked='checked' />范围外</td> 							     
						     </tr>
						     <tr>
								<td colspan="2" style="text-align:center;">
