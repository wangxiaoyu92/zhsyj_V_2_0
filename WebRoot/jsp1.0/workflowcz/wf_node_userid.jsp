...
<script type="text/javascript">
	...
	function queryWfnodeRole() {		
		...
		grid.datagrid({
			url : basePath + '/workflow/queryWfnodeUserid',
			queryParams : param
		});
	...
	var addWfnodeRole = function() {
		if(nodeid == "" || nodeid == null){
			...
			var dialog = parent.sy.modalDialog({
				...
				height : 500,
				maximizable : false,
				url : basePath + '/workflow/wf_node_roleAddUseridIndex?psbh=' + psbh + '&nodeid=' + nodeid,
				buttons : [ {
					text : '确定',
					handler : function() {
						dialog.find('iframe').get(0).contentWindow.submitForm(dialog, grid, parent.$);
	        		...
	        		<table>
						<tr>	        		
							...
							<td><a href="javascript:void(0)" class="easyui-linkbutton" data="btn_delWfnodeRole"
								iconCls="icon-remove" plain="true" onclick="delWfnodeUserid()">删除节点权限</a>
							</td>   
							<td>
								<div class="datagrid-btn-separator"></div>
							</td>  																				
