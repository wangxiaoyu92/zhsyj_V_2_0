...
<%
    ...
    String t_catejc = StringHelper.showNull2Empty(request.getParameter("catejc"));
%>
<!DOCTYPE html>
<html>
<head>
<title>安盛食品安全追溯平台-新闻列表</title>
...
<script type="text/javascript">
    ...
    var options = {
        ...
        tooltipTitles: function (type, page, current) {
            ...
            getNewsList(page, v_pageSize);
        }
    }
    
	$(function() {
		$('#currlocation').html(t_currlocation); 	
		getNewsList(1, v_pageSize);   
	});
	// 获取新闻列表
	function getNewsList(page,pageSize) {
    	$.ajax({
        	...
        	type: "post",
            dataType: 'json',
            data: { page: page, rows: pageSize, catejc: t_catejc },
            	...
            	if(result.code == '0'){
            		options.currentPage = result.currPage;
                 	...
                 	$.each(result.rows, function (index, item) {		                
                    	var html = "";
						html += " <li><a href=\"javascript:linkToSelf('news_detail.jsp?newsid=" + item.newsid + "');\">" + getPartContent(item.newstitle,50) + "<span style=\"text-align:right\">" + item.newstjsj + "</span></a></li>";
                    	$('#content').append(html);
