...
<html>
  <head>
    ...
    <script type="text/javascript">
	 ...
	 $(function  (){	
  		grid=$("#grid").datagrid({
	    url: basePath +"/tmsyjhtgl/queryJgztpicList?jgztpickind=1" , 
    	iconCls:'icon-ok',
    	pageSize:10,
    	pageList:[10,20,30],
   ...
   //新增客户关系
    ...
    var addkhgx=function(){  
	     var dialog =  parent.sy.modalDialog({
			...
			width : 400,
			height : 420,
			url : basePath + '/tmsyjhtgl/zcbUploadFormIndex?op=add',
			buttons : [ {
				text : '确定',
				handler : function() {
   ...
   //编辑客户关系
    var editkhgx=function(){
       ...
       if(row){
	     var dialog =  parent.sy.modalDialog({
			...
			width : 640,
			height : 360,
			url : basePath + '/tmsyjhtgl/zcbUploadFormIndex?op=edit&hjgztxgpicid='+row.hjgztxgpicid,
			buttons : [ {
				text : '确定',
				handler : function() {
					dialog.find('iframe').get(0).contentWindow.submitForm(dialog, grid, parent.$);
    ...
    var showkhgx=function(){
       ...
       if(row){
	     var dialog =  parent.sy.modalDialog({
			...
			width : 640,
			height : 360,
			url : basePath + '/tmsyjhtgl/zcbUploadFormIndex?op=view&hjgztxgpicid='+row.hjgztxgpicid,
			buttons : [ {
				text : '取消',
				handler : function() {
					dialog.find('iframe').get(0).contentWindow.closeWindow(dialog, parent.$);
	...
	function uploadFjView(){
		var row = $('#grid').datagrid('getSelected');
		if (row) {
			var url = basePath + "/pub/pub/uploadFjViewIndex?folderName=zcbpic&fjwid="+row.hjgztxgpicid+"&fjtype=15";//15自查表图片 
			//创建模态窗口
			var dialog = parent.sy.modalDialog({
				title : '上传附件',
				width : 900,
			...
			<table> 
				<tr>
					...
					<div class="datagrid-btn-separator"></div>
							...
							iconCls="icon-upload" plain="true" onclick="uploadFjView()">上传自查图片</a>
						</td>					
				</tr>
			</table>
			</div> 
