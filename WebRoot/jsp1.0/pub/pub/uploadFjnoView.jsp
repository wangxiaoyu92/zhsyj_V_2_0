...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#uploader").pluploadQueue({
			...
			url : '<%=contextPath%>/pub/pub/uploadFj',//上传文件URL
			...
			filters : [ {
				...
				{ title:'视频', extensions:'flv,mp4' }],
			init : {
				FileUploaded : function(up, file, info) { //文件上传完毕触发
					comfjpath = uploadPath + folderName + "/" + file.name;
				}
			}
		});	
	});
			...
			if (uploader.files.length > 0) {// 判断队列中是否有文件需要上传
				uploader.bind('StateChanged', function() {// 在所有的文件上传完毕时，提交表单
					if (uploader.files.length === (uploader.total.uploaded + uploader.total.failed)) {
						...
						var s = new Object();      
						s.type = "ok";
						s.comfjpath = comfjpath;
						s.uploadurl = uploadPath + folderName;
						sy.setWinRet(s);
