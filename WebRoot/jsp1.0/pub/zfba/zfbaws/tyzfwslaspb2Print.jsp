...
<div style="width: 210mm; margin: 0 auto;display: none">
	   <body class="b1 b2">
		...
		<div class="Noprint">
			...
			<p class="p3">
				<span class="s2">案 由：<span style="border-bottom: 1px solid  black;width: 560px;display: inline-block;"><%=localZfwslaspb2DTO.getAjdjay()%></span>
				 </span>
			</p>
			<p class="p3">
				<span class="s2">当事人：<span style="border-bottom: 1px solid  black;width: 240px;display: inline-block;"><%=localZfwslaspb2DTO.getLaspdsr()%></span>
				 </span> 
				 法定代表人（负责人）：<span style="border-bottom: 1px solid  black;width: 150px;display: inline-block;">
				 <%=localZfwslaspb2DTO.getLaspfddbr()%>
				 </span>
			</p>
			<p class="p4">
				<span class="s2">地 &nbsp;&nbsp;址：<span style="border-bottom: 1px solid  black;width: 240px;display: inline-block;"><%=localZfwslaspb2DTO.getLaspdz()%></span> 
				 </span> 
				 联系方式：<span style="border-bottom: 1px solid  black;width: 230px;display: inline-block;">
				 <%=localZfwslaspb2DTO.getLasplxfs()%>
				  </span>
			</p>
			<p class="p4">
				<span class="s2">案件来源：
				<span style="border-bottom: 1px solid  black;width: 300px;display: inline-block;"><%=localZfwslaspb2DTO.getAjdjajlystr()%></span>
			 </span>
			</p>
			<hr style="height:2px;border:none;border-top:2px solid #555555;" />
			<p class="p5">
			...
			<p class="p8">
				...
				<span class="s4"><%=localZfwslaspb2DTO.getLaspflfg()%></span>
					的规定，申请予以立案。
			...
			<p class="p10">
				...
				<%if("".equals(localZfwslaspb2DTO.getLaspjbr())||
				localZfwslaspb2DTO.getLaspjbr().length()==0) {%> 
					<%}else{ %>
					<%=localZfwslaspb2DTO.getLaspjbr()%> 
					<%} %>
				<%if("".equals(localZfwslaspb2DTO.getLaspjbr2())||
				localZfwslaspb2DTO.getLaspjbr2().length()==0) {%> 
					<%}else{ %>、
					<%=localZfwslaspb2DTO.getLaspjbr2()%>
					<%} %>  
					</span>
					</span>
			</p>
			...
			<p class="p8">
				<span class="s2">建议本案由 
					<span class="s4"> 
					<%=localZfwslaspb2DTO.getLaspjyzb()%> </span>主办。</span>
				<span class="s2">由  <span class="s4"> <%=localZfwslaspb2DTO.getLaspjyxb()%> </span> 协办。</span><br>
			</p> 
			</p>
			<p class="p10">
			 <span class="s2"> 承办部门负责人：
				<span style="border-bottom: 1px solid  black;width: 120px;display: inline-block;text-align: left;"> 
				<%if("".equals(localZfwslaspb2DTO.getLaspcbbmfzr())||
				localZfwslaspb2DTO.getLaspcbbmfzr().length()==0) {%>
					 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<%}else{ %>
			...
			<p class="p12" style="height: 28px; ">
				<span class="s3"> 审批意见：</span> 
			</p> 
			<p class="p6"><br>
			<span class="s4"><%=localZfwslaspb2DTO.getLaspspyj() %></span>
			<span class="s2">  自，<span class="s4">
				<%if(!"".equals(localZfwslaspb2DTO.getLaspsplarq()) 
					&& localZfwslaspb2DTO.getLaspsplarq().length() >= 10){ %>
					 <%=localZfwslaspb2DTO.getLaspsplarq().substring(0,4)%>
					年<%=localZfwslaspb2DTO.getLaspsplarq().substring(5,7)%>
					月<%=localZfwslaspb2DTO.getLaspsplarq().substring(8,10)%>
					<%}else{%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;年 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					月&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <%}%>
					</span>日起，
				  </span>
				<span class="s2">建议由 
				 <span class="s4"> <%=localZfwslaspb2DTO.getLaspspzb()%> </span> 主办。</span>
				<span class="s2">由 
				 <span class="s4"> <%=localZfwslaspb2DTO.getLaspspxb()%> </span> 协办。
				</span>
			</p>
			<p class="p13">
				<span class="s2"> </span>
			...
			<p class="p10">
				<span class="s2"> 分管负责人：
				    <span style="border-bottom: 1px solid  black;width: 120px;display: inline-block;text-align: left;"> 
				    <%if("".equals(localZfwslaspb2DTO.getLaspfgfzr())||
				     localZfwslaspb2DTO.getLaspfgfzr().length()==0) {%>
					 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<%}else{ %>
