...
<body class="b1 b2">
	...
	<div align="center" id="page1">
		...
		<p class="p4">
			...
			<%=StringHelper.myMakeStrLen(dto.getXxcldsr(), 80, 0)%>:</span>
		</p>
		<p class="p5 l24">
			<span class="s2">我局于</span>
			<span class="s3">
		    <%if(dto.getXxclcfkyrq() != null && !"".equals(dto.getXxclcfkyrq())){ %>
	    	<%=StringHelper.myMakeStrLen(dto.getXxclcfkyrq().substring(0,4), 6, 1)%><%} else {%>
	    	<%=StringHelper.myMakeStrLen("", 6, 1)%><%} %>
	    	</span>
			<span class="s2">年</span>
			<span class="s3">
			<%if(dto.getXxclcfkyrq() != null && !"".equals(dto.getXxclcfkyrq())){ %>
			<%=StringHelper.myMakeStrLen(dto.getXxclcfkyrq().substring(5,7), 4, 1)%><%} else {%>
			<%=StringHelper.myMakeStrLen("", 4, 1)%><%} %>
			</span>
			<span class="s2">月</span>
			<span class="s3">
			<%if(dto.getXxclcfkyrq() != null && !"".equals(dto.getXxclcfkyrq())){ %>
			<%=StringHelper.myMakeStrLen(dto.getXxclcfkyrq().substring(8,10), 4, 1)%><%} else {%>
			<%=StringHelper.myMakeStrLen("", 4, 1)%><%} %>
			</span>
			<span class="s2">日以</span>
			<span class="s3">
			<%=StringHelper.myMakeStrLen(dto.getXxclcfkyjdsbh(), 30, 0)%></span>
			<span class="s2">《查封（扣押）决定书》查封（扣押）了你（单位）的物品。为防止造成不必要的损失，
			根据《食品药品行政处罚程序规定》第二十九条第二款的规定，本局决定对
			<span class="s3">
			<%=StringHelper.myMakeStrLen(dto.getXxclwplb(), 26, 0)%></span>
