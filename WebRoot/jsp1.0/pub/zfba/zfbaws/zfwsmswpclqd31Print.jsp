...
<% 
    ...
    if (request.getAttribute("printbean") != null) {
    	dto = (Zfwsmswpclqd31DTO) request.getAttribute("printbean");
    }	
...
function CreatePrintPage() {
	...
	LODOP.ADD_PRINT_HTM("265mm","20mm","85%","100%",strBodyStyle+document.getElementById("footer").innerHTML);
}
</script>
...
<body class="b1 b2">
	...
	<div align="center" id="page1">
		...
		<div align="right">
			<p class="p3"><span class="s2"><%if (dto.getMsclwsbh() != null) { %>
			<%=dto.getMsclwsbh() %><%} %></span>
			</p>
		</div>
		<hr style="height:2px;border:none;border-top:2px solid #555555;" />
		<p class="p4">
			<span class="s2">根据</span>
			<%if(dto.getMsclxzcfjdsbh() != null && !"".equals(dto.getMsclxzcfjdsbh())){ %>
			<span><%=dto.getMsclxzcfjdsbh()%></span><%} else {%>
			<span style="border-bottom: 0px solid  white-space;width: 280px;display: inline-block;"></span><%} %>
			<span class="s2">《行政处罚决定书》</span>
		</p>
		<p class="p5">
			<span class="s2">当事人：</span>
			<span style="border-bottom: 0px solid  white-space;width: 210px;display: inline-block;">
			<%if (dto.getMscldsr() != null) { %> <%=dto.getMscldsr()%><%} %></span>
			<span class="s2">地 址：</span>
			<span style="border-bottom: 0px solid  white-space;width: 180px;display: inline-block;">
			<%if (dto.getMscldsrdz() != null) { %> <%=dto.getMscldsrdz()%><%} %></span>
			<span class="s2">电话：</span>
			<span style="border-bottom: 0px solid  white-space;display: inline-block;">
			<%if (dto.getMscldsrdh() != null) { %> <%=dto.getMscldsrdh() %><%} %></span>
			
		</p>
		<p class="p5">
			<span class="s2">执行处置单位：</span>
			<span style="border-bottom:0px solid  white-space;width: 170px;display: inline-block;">
			<%if (dto.getMsclzxczdw() != null) { %> <%=dto.getMsclzxczdw() %><%} %></span>
			<span class="s2">地 址：</span>
			<span style="border-bottom: 0px solid  white-space;width: 180px;display: inline-block;">
			<%if (dto.getMsclczdwdz() != null) { %> <%=dto.getMsclczdwdz()%><%} %></span>
			<span class="s2">电话：</span>
			<span style="border-bottom: 0px solid  white-space;display: inline-block;">
			<%if (dto.getMsclczdwdh() != null) { %> <%=dto.getMsclczdwdh() %><%} %></span>
		</p>
		<hr style="height:2px;border:none;border-top:2px solid #555555;" />
		<p class="p6">
		...
		<table class="t1" id="mswpmx">
			...
			<%  }
			if ((v_list.size() < 19) || (v_list.size() > 19 && v_list.size() < 40)) {
				int num = v_list.size() - 19 > 0?v_list.size() - 19:v_list.size();
				for (int k = num; k < 19; k++){ %>
		...
		<p class="p5">
			...
			<span style="border-bottom: 1px solid  black;width: 120px;display: inline-block;"></span>（签字）
			<span style="border-bottom: 0px solid  white-space;width: 50px;display: inline-block;"></span>
			<span class="s2" >承办人：</span>
			<span style="border-bottom: 1px solid  black;width: 150px;display: inline-block;"></span>（签字）
		</p>
		<p class="p7">
			<span class="s2">
			    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;年
