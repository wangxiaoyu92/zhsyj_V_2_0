...
public class PubFunc {
	...
	public static double parseDouble(Object s) {
		try {
			if (s == null || ((String) s).trim().equals(""))
				...
				return Double
						.parseDouble(replace(((String) s).trim(), ",", ""));
		} catch (Exception e) {
			return 0;
		}
	...
	public static String formatDouble(double s, int digit) {
		/*
		 * int a=1; for (int i=0; i<digit; i++) { a=a*10; } return
		 * String.valueOf((double)(((long)(s*a))/a));
		 */
	...
	public static Enumeration getConfList(String conffile, String item) {
		...
		try {
			...
			if (debug)
				System.out.println("读文件" + conffile + "出错!" + ex.getMessage());
			return null;
		}
		return ar.elements();
	...
	/* 读取配制项匹配的记录值 */
	public static String readConfItem(String conffile, String condition,
			String item) {
		if ((condition == null) || (condition.equals("")))
			...
			while (str != null) {
				if (str.equals(condition)) {
					while ((str != null) && (!str.equals(endline))) {
						if (str.startsWith(item + "=", 0)) {
							found = true;
							value = str.substring(new String(item + "=")
									.length());
							str = null; // 找到,结束
						} else
							str = is.readLine();
	...
	public static boolean writeFile(String filename, String filecontext) {
		...
		try {
			...
			if (debug)
				System.err.println("不能写文件: " + filename);
			return false;
		}
	}
	...
	public static boolean writeConf(String conffile, String item, String value) {
		...
		try {
			...
			if (debug) {
				...
				System.out.println("rename file " + conffile + " to "
						+ conffile + ".bak");
				System.out.println(moveFile(conffile, conffile + ".bak"));
				System.out.println("rename file " + conffile + ".tmp to "
						+ conffile);
	...
	public static String strOrLike(String fieldValue, String field) {
		if (isNullStr(fieldValue))
			...
			return " or " + field + " like '%" + toSqlStr(fieldValue.trim())
					+ "%' or '" + toSqlStr(fieldValue.trim()) + "' like '%'+"
					+ field + "+'%' ";
	}
	...
	public static String getInputParam(String[] query_type) {
		...
		if (query_type != null)
			for (int i = 0; i < query_type.length; i++)
				s = s + "<input type='hidden' name='query_type' value='"
						+ query_type[i] + "'>";
		return s;
	}
	...
	public static String getInputField(String inputfield, String inputvalue) {
		if (inputvalue != null)
			return "<input type='hidden' name='" + inputfield + "'"
					+ " value='" + inputvalue + "'>";
		else
			return "<input type='hidden' name='" + inputfield + "'>";
	}
	...
	public static String getLegalDate(String year, String month, String day,
			String hour, String minute, String secound) {
	...
	public static String getLastDay(String FormatDate) {
		try {
			...
			return FormatDate.substring(0, 4) + "/"
					+ FormatDate.substring(5, 7) + "/" + String.valueOf(maxDay);
		} catch (Exception e) {
			return "1970/01/01";
		}
	}
	...
	public static String beforeMonth(String yearmonth) {
		if (yearmonth.length() == 6) {
			String s = yearmonth.substring(0, 4) + "/"
					+ yearmonth.substring(4, 6) + "/01";
...
/**
	...
	/**
	 * 缩写字符串:调用方法 ${fnx:abbreviate('要截取的源字符串', '字节长度', '尾部追加字符')}
	 * @param str:要缩写的原始source string
